...
import sys
import shutil
from sty import fg  # used to style colors
...
def show_or_hide_terminal(commandarray: list[str], shell: bool = False) -> int:
    global showTerminal
...
def navigate_to_cactbot(_state: str) -> None:
    debug_print("Navigate to Cactbot dir")
    dir_path = os.path.dirname(os.path.realpath(__file__))
    os.chdir(dir_path)  # get to a known location
    if _state == "aku":
        try:
            if "cactbot_aku2" not in dir_path:
                os.chdir("./cactbot_aku2")  # navigate to cactbot dir
        except:
            pass
        try:
...
def build_cacbot_config() -> None:
    ...
    try:
        ...
        output: int = show_or_hide_terminal(command, shell=True)
    except Exception:
        output = 9999
    if int(output) > 0:
...
def build_cacbot() -> None:
    ...
    try:
        ...
        debug_print(" ".join(command))
        output: int = show_or_hide_terminal(command)
    except Exception:
        output = 1
        pass
...
def publish_cactbot():
    ...
    addon_path: LiteralString = f"{root_path}/addons"
    cacbot_path: LiteralString = f"{root_path}/cactbot"
    ...
    shutil.copy2(
        "./bin/x64/Release/CactbotEventSource.dll", f"{cacbot_path}/CactbotEventSource.dll"
    )
...
def copy_to_act() -> None:
    ACT_cacbot_path = "G:/FINAL FANTASY XIV - A Realm Reborn/AdditionalExtern/ACT/Plugins/OverlayPlugin_ngld/cactbot"
    try:
    ...
    try:
        shutil.copy2(
            "./bin/x64/Release/CactbotOverlay.dll", f"{ACT_cacbot_path}/CactbotOverlay.dll"
        )
        shutil.copy2(
            "./bin/x64/Release/CactbotEventSource.dll", f"{ACT_cacbot_path}/CactbotEventSource.dll"
        )
...
def copy_to_oopsy_webserver() -> None:
    try:
...
def copy_to_raidemulator_webserver() -> None:
    try:
        raidemulator_cacbot_path = r"\\192.168.2.4\Root\var\www\ffxiv"
        shutil.copy2(
            "./ui/raidboss/raidemulator.html",
            f"{raidemulator_cacbot_path}\\raidemulator\\raidemulator.html",
        )
        shutil.copy2(
            "./ui/raidboss/raidemulator.css",
            f"{raidemulator_cacbot_path}\\raidemulator\\raidemulator.css",
        )
        shutil.copy2(
            "./resources/defaults.css", f"{raidemulator_cacbot_path}\\resources\\defaults.css"
        )
...
def copy_cacbot_nightly_zip() -> None:
    try:
